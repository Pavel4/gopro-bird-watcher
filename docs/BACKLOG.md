...
**DONE:** Реализовано в `feeder_analytics.py`.
Параметры конфигурации: `ANALYTICS_ENABLED`,
`ANALYTICS_DIR`, `DEFAULT_FOOD_TYPE`.
Telegram-команды: `/stats`, `/stats week`,
`/stats food`, `/stats hours`, `/food <тип>`.
...
**DONE:** Базовая реализация в `feeder_analytics.py`.
CSV-экспорт (visits.csv, food_log.csv, daily_stats.csv),
Telegram-команды `/stats`, `/food`.
